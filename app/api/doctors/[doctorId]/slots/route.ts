...
export async function GET(
  ...
  try {
    ...
    const doctor = store.doctors.getById(doctorId);
    if (!doctor) {
      return errorResponse("Doctor not found", 404);
    }

    const slotList = store.slots.getByDoctorAndDate(doctorId, date);
    const slots = slotList
      .sort((a, b) => new Date(a.startTime).getTime() - new Date(b.startTime).getTime())
      .map((slot) => {
        const tokensInSlot = store.tokens.getBySlot(slot.id);
        const waitlistEntries = store.waitlist
          .getByDoctor(doctorId)
          .filter((w) => w.preferredSlotId === slot.id && w.status === "waiting");
        return {
          slotId: slot.id,
